...
import React, { useState, useEffect } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { FaSearch, FaArrowRight, FaBuilding, FaUniversity, FaUsers, FaUser, FaBookOpen } from "react-icons/fa";
...
const SearchBar = () => {
  ...
  const renderHierarchyView = () => {
    ...
    const availableLevels = hierarchyLevels.filter(level => hierarchyData[level]);
    
    return (
      <div className="mt-4 rounded-md bg-white p-6 shadow-md">
        <h3 className="mb-6 text-xl font-semibold text-gray-800">Hierarchical View</h3>
            ...
            return (
              <div key={level} className="relative">
                ...
                <div className="flex items-center rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                  ...
                  <div>
                    <div className="text-xs font-medium uppercase text-gray-500">{getLabelForType(level)}</div>
                    ...
                    {item.contact && <div className="text-sm text-gray-600">{item.contact}</div>}
